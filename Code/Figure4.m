...
for index = 1:3
    if isfile(sprintf("CyclePeriod=%.1f,S0=%1.e,V0=%1.e,p_L=%.1f,p_V=%.1f.mat",CyclePeriod,S0,V01,p_LV(index,1),p_LV(index,2)))
        load(sprintf("CyclePeriod=%.1f,S0=%1.e,V0=%1.e,p_L=%.1f,p_V=%.1f.mat",CyclePeriod,S0,V01,p_LV(index,1),p_LV(index,2)));
...
for index = 1:3
    ...
    if isfile(sprintf("InvasionCyclePeriod=%.1f,S0=%1.e,V0=%1.e,p_L=%.1f,p_V=%.1f.mat",CyclePeriod,S0,V01,p_LV(index,1),p_LV(index,2)))
        load(sprintf("InvasionCyclePeriod=%.1f,S0=%1.e,V0=%1.e,p_L=%.1f,p_V=%.1f.mat",CyclePeriod,S0,V01,p_LV(index,1),p_LV(index,2)));
...
%% Save Figure
filename = dir('SteadyStatePlusInvasion*');
filename = filename(end).name;
if isempty(filename)
    filename = 'SteadyStatePlusInvasion_v1.eps';
else
