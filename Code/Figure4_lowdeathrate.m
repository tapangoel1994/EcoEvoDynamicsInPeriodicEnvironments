%% This script generates Figure 4 for the revised manuscript.
...
%% Note: Run script DataGenerate_lowdeathrate before running this script.
...
for deathrateidx = 1
    ...
    for qV = [.2 0]
        for cycleperiod = [24 48]
            ...
            yticks(10.^(-6:1:-1));
            yticklabels(arrayfun(@(x)sprintf("10$^{%d}$",x),-6:1:-1));
    ...
    plot(OptimalStrategy(OptimalStrategy.q_L == .2 & OptimalStrategy.q_V == 0 & OptimalStrategy.CyclePeriod == 48,:).CyclePeriod, ...
        OptimalStrategy(OptimalStrategy.q_L == .2 & OptimalStrategy.q_V == 0 & OptimalStrategy.CyclePeriod == 48,:).PMax, ...
    ...
    set(gca,'XTick',0:12:96,'XTickLabel',0:12:96,'YLim',[0 1],'YTick',0:.1:1,'FontSize',14,'FontWeight','bold','TickLabelInterpreter','latex','Box','off');
    ...
    semilogy(OptimalStrategy(OptimalStrategy.q_L == .2 & OptimalStrategy.q_V == 0 & OptimalStrategy.CyclePeriod == 48,:).CyclePeriod, ...
        OptimalStrategy(OptimalStrategy.q_L == .2 & OptimalStrategy.q_V == 0 & OptimalStrategy.CyclePeriod == 48,:).GammaMax, ...
        'LineStyle','none','Marker','o','MarkerFaceColor','r','MarkerEdgeColor','r','MarkerSize',10);
    
    lgd = legend(arrayfun(@(x)sprintf('$q_L = %.3f$',x),q_LV_sub(:,1),'UniformOutput',false),'Interpreter','latex','Location','southeast','FontSize',14,'Box','off');
    set(gca,'XTick',0:12:96,'XTickLabel',0:12:96,'YLim',[5e-7 1e-1],'YTick',10.^(-6:1:-1),'FontSize',14,'FontWeight','bold','TickLabelInterpreter','latex','Box','off', 'YMinorTick','off');
    xlabel('Cycle period (hr)','FontSize',16,'Interpreter','latex','FontWeight','bold');
...
%% Save figure
saveas(h,'../RevisedFigures/Figure4.svg');